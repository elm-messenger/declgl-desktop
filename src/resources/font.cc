...
bool Font::parse(const char* bytes, std::size_t size) {
    ...
    // ---- distanceField ----
    if (auto df_it = doc.find("distanceField"); df_it != doc.end() && df_it->is_object()) {
    ...
    // ---- kernings (optional) ----
    if (auto kern_it = doc.find("kernings"); kern_it != doc.end() && kern_it->is_array()) {
...
const Glyph* Font::find_glyph(char c) const {
    return find_glyph_by_id(static_cast<int>(static_cast<unsigned char>(c)));
}
