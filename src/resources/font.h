...
class Font {
	...
	int kerning_count() const
	{
		return static_cast<int>(kernings_.size());
	}

	// Cached `distanceRange / scaleW` and `distanceRange / scaleH`,
	// i.e. the exact `unitRange` uniform the JS shader expects.
	float unit_range_x() const
	{
		return distance_range_ / scaleW_;
	}
	float unit_range_y() const
	{
		return distance_range_ / scaleH_;
	}
