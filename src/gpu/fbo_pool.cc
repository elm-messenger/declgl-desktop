...
void FboPool::destroy_all() {
    for (auto& f : fbos_) {
        if (f.framebuffer) glDeleteFramebuffers(1, &f.framebuffer);
...
void FboPool::release(int id) {
    if (id < 0 || id >= static_cast<int>(free_.size())) return;
    free_[static_cast<size_t>(id)] = true;
}

const Fbo* FboPool::get(int id) const {
    if (id < 0 || id >= static_cast<int>(fbos_.size())) return nullptr;
